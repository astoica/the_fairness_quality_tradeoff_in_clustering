''' This code implements the repeated FCBC algorithm (Algorithm 2) with the group utilitarian objective. 
...
if __name__ == "__main__":
    ...
    for alpha_POF in alpha_POF_values: 
        ...
        group_utilitarian_obj[alpha_POF] = util_objective 


    with open('output/' + which_data + 'clustering_cost' + str(max_points) + '_numclusters'+ str(num_cluster) + '_grouputil.json', 'w') as f:
        json.dump(clustering_cost, f)

    # Writing dictionary to a JSON file
    with open('output/' + which_data + 'group_utilitarian_obj' + str(max_points) + '_numclusters'+ str(num_cluster) + '.json', 'w') as f:
        json.dump(group_utilitarian_obj, f)
